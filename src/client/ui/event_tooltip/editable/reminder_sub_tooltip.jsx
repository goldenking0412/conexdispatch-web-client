...
export default class ReminderSubTooltip extends React.Component {
    ...
    remove_reminder(event) {
        const minutes = $(event.target)
            .closest("tr[data-reminder-minutes]")
    ...
    add_reminder(event) {
        ...
        const is_dupplicate =
            _.find(this.props.reminders, { minutes: reminder_minutes }) !== undefined;
    ...
    render() {
        return (
            <DetailsSubTooltip title="Alerts" toggle_subtooltip={this.props.toggle_subtooltip}>
                <div className="reminder-subtooltip">
                    <form onSubmit={this.add_reminder}>
                        <select
                          ref={ref => {
                              this.event_reminder_select = ref;
                          }}
                            ...
                            {_.map(this.available_reminders, reminder => {
                                return (
                                    <option key={reminder.minutes} value={reminder.minutes}>
                                        {moment().add(reminder.minutes, "minute").fromNow(true)}
                                        {" "}
                                        before event
                                    </option>
                                );
                ...
                {_.map(props.reminders, reminder => {
                    return (
                        <tr key={reminder.minutes} data-reminder-minutes={reminder.minutes}>
                            <td colSpan="2">
                                {moment().add(reminder.minutes, "minute").fromNow(true)} before
                                event
                            </td>
                            <th>
                                <button
