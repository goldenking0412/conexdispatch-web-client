...
export function read_query_string(query_string) {
    return _(query_string).split("&").map(item => _.split(item, "=")).fromPairs().value();
}
...
export function setup_message_popup() {
    $(document).on("click", ".message-close", function close_popup() {
...
export function create_message_popup(message, classes = [], delay = 2000) {
    ...
    $message.addClass(classes.join(" ")).insertAfter($message_template);
